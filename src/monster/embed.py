...
class MonsterEmbed(discord.Embed):
   ...
   headers: dict
    ...
    def main(self):
        ...
        embed.add_field(name=self.h['ailments'], value=join_char.join(self.dct['ailments']), inline=True)
        embed.add_field(name=self.h['inmune'], value=join_char.join(self.dct['inmune']), inline=True)
        embed.add_field(name=self.h['weakness-3'], value=join_char.join(self.dct['weakness-3']), inline=True)
        embed.add_field(name=self.h['weakness-2'], value=join_char.join(self.dct['weakness-2']), inline=True)
        embed.add_field(name=self.h['weakness-1'], value=join_char.join(self.dct['weakness-1']), inline=True)
        embed.add_field(name=self.h['breakable'], value=join_char.join(self.dct['breakable']), inline=True)
        embed.add_field(name=self.h['location'], value=join_char.join(self.dct['locations']), inline=True)
        ...
        if self.dct['has-alt-weakness']:
            ...
            embed2.add_field(name=self.h['inmune'], value=join_char.join(self.dct['alt-inmune']), inline=True)
            embed2.add_field(name=self.h['weakness-3'], value=join_char.join(self.dct['alt-weakness-3']), inline=True)
            embed2.add_field(name=self.h['weakness-2'], value=join_char.join(self.dct['alt-weakness-2']), inline=True)
            embed2.add_field(name=self.h['weakness-1'], value=join_char.join(self.dct['alt-weakness-1']), inline=True)
            embed.timestamp = datetime.datetime.now()
            embed.set_footer(text=format_uppercase(self.dct['name']))
        return embed, embed2
